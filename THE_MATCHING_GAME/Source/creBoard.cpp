    ...
    for (int i = 0; i <size; i++)
    {
        for (int j = 0; j <size; j++)
        {
            if (front[i][j] != ' ')
            {
                ...
                /*for (int y = 10 + 4 * (i + 1); y > 10 + 4 * (i); y--)
                {
                    gotoxy(46 + j * 8, y);
                    std::cout << "|";//"|";
                }
                for (int k = 46 + ((j + 1) * 8); k > 46 + (j) * 8; k--)
                {
                    gotoxy(k, 10 + 4 * i);
                    std::cout << "+";//<< "-";
                }
            ...
            else
            {
                ...
                if (((i < (size - 1))&&(front[i + 1][j] == ' '))||(i==size-1))
                {
                    for (int k = 46 + ((j + 1) * 8); k > 46 + (j) * 8; k--)
                    {
                        gotoxy(k, 10 + 4 * (i + 1));
...
bool checkL1(int** core, point targetA, point targetB) //check I nh?ng không quan tâm bi?u t??ng
{
    int A, B;
...
bool checkZ1(int** core, char** front, point targetA, point targetB)// check L nh?ng không quan tâm bi?u t??ng
{
    point corner1, corner2;
...
bool checkZ(int** core, char** front, point targetA, point targetB) {
    ...
    if (targetA.y > targetB.y)// tìm s? có tung ?? nh? h?n
    {
        A = targetB;
        B = targetA;
    }
...
void AdvancedMode(int**& core, char**& front, int size,int*a)
{
    ...
    direction = 1 + rand() % 4; // L?a ch?n ng?u nhiên 1 trong 4 ki?u hard mode: 1-> d?n sang trái, 2 -> ph?i, 3 -> lên, 4 -> xu?ng 
    while (true)
    {
        switch (check)
        {
...
void suggestion(int** core, char** front, int size,int * a)
{
    ...
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            ...
            if (core[i][j] == 1 || core[i][j] == 2) // ki?m tra xem có  b?ng nhau không
            {
...
bool checkWin(int** core, char** front, int size) // ki?m tra xem th? n?u không có c?p nào th?a thì t?o b?ng l?i
{
    int check = 0;
    point targetA, targetB;
...
void randomAgain(int**& core, char**& front, int size) // khi không còn ???ng ?i thì random m?t l?n n?a các kí t? còn l?i
{
    srand((int)time(0));
